...
public interface ClassEntityService {
     ...
     * @return 分类响应对象列表
     */
    List<ClassResponse> selectByStatus(Integer status);


    /**
     * 根据用户 ID 和状态查询分类响应对象列表
     * @param userId 用户 ID
     * @param status 分类状态
     * @return 分类响应对象列表
     */
    List<ClassEntity> selectByUserIdAndStatus(boolean selectMain, Long userId, Integer status);
