...
public class DoubaoAiServiceImpl implements AiService {
    ...
    private String buildPromptWithCategories(String prefix, Long userId) {
        ...
        promptBuilder.append("，按要求返回JSONObject（如果有多个对象就汇总成一个JSONObject再返回，无多余文本）：1.memo：一句话描述商品/商家/支出/退款/收入等 2.amount：金额（退款不计） 3.datetime：时间转YYYY-MM-dd HH:mm:ss，无则null 4.reply：可爱搞笑风格或温暖慰藉风格女孩回复 5.category：选最匹配的ID（分类列表：");
        
        // 查询所有 正常状态下 最底层的分类（type=2）
        try {
            List<ClassEntity> classList = classEntityService.selectByUserIdAndType(false, userId, 2);
