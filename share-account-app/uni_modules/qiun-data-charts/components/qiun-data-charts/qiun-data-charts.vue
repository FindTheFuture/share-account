...
export default {
  ...
  created(){
    ...
    // 兼容微信端新API，优先使用 getWindowInfo/getDeviceInfo（仅在缺失时回退到 getSystemInfoSync）
    const getSys = () => {
      try {
        const hasWin = typeof uni.getWindowInfo === 'function';
        const hasDev = typeof uni.getDeviceInfo === 'function';
        let platform; let pixelRatio;
        if (hasDev) {
          const dev = uni.getDeviceInfo();
          platform = dev && dev.platform;
        }
        if (hasWin) {
          const win = uni.getWindowInfo();
          pixelRatio = win && win.pixelRatio;
        }
        if (!platform || !pixelRatio) {
          const s = typeof uni.getSystemInfoSync === 'function' ? uni.getSystemInfoSync() : {};
          platform = platform || s.platform;
          pixelRatio = pixelRatio || s.pixelRatio;
        }
        return { platform, pixelRatio };
      } catch (e) {
        const s = typeof uni.getSystemInfoSync === 'function' ? uni.getSystemInfoSync() : {};
        return { platform: s.platform, pixelRatio: s.pixelRatio };
      }
    };
    const systemInfo = getSys();
            ...
            //如果是小程序端，采用uCharts渲染
              ...
              this.$nextTick(()=>{
                if (this.type2d === true) {
                  ...
                  cfu.option[cid].context = uni.createCanvasContext(cid, this.$scope || this);
                  if(cfu.instance[cid] && cfu.option[cid] && cfu.option[cid].update === true){
                    this._updataUChart(cid)
                  }else{
                    setTimeout(()=>{
